...
public class ConfigManager implements Listener{
    ...
    public void loadConfigs(Plugin plugin, File file, FileConfiguration cfile, CommandSender sender) {
        ...
        if(!file.exists()) {
            ...
            if(sender != null) {
                sender.sendMessage( ChatColor.RED + fileName + " does not exist. Creating it for you.");
            }
        } else {
            try {
                cfile.load(file);
                if(sender != null) {
                    sender.sendMessage(ChatColor.GREEN + fileName + " successfully reloaded!");
                }
            } catch (IOException | InvalidConfigurationException e) {
                e.printStackTrace();
                if(sender != null) {
                    sender.sendMessage(ChatColor.RED + fileName + " could not be reloaded! Check console for errors.");
                }
            }
        }
    }
